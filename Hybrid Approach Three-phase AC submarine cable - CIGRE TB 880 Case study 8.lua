...
-- -------------------------
conductor_diameter = 38.6               -- Conductor diameter 
conductor_tape_diameter = 39            -- External diameter of conductor tape
conductor_screen_outer_diameter = 42    -- External diameter of sc conductor screen
insulation_outer_diameter = 88          -- External diameter of insulation
insulation_screen_outer_diameter = 90.8 -- External diameter of sc insulation screen
water_blocking_outer_diameter = 92.2    -- External diameter of sc water blocking tape
lead_sheath_outer_diameter = 99.4       -- External diameter of metallic (lead) sheath
pe_sheath_outer_diameter = 104.6        -- External diameter of sc PE sheath
filler_diameter = 226                   -- External diameter of fillers (cable after assembling)
cable_core_binder_diameter = 226.6      -- External diameter of cable core binder
armour_bedding_diameter = 227           -- External diameter of armour bedding
armour_outer_diameter = 237             -- External diameter of the armour
cable_outer_diameter = 245              -- External diameter of outer covering
...
number_armour_wires = 133 -- Number of Armour wires
diameter_armour_wires = 5  -- Diameter of Armour wires (mm)
armour_lay_length = 2900 -- Lay length of the Armour (mm)

-- System parameters 
Current = 1135                         -- Steady state current (A)
...
system_voltage = 220000                -- System voltage (V)
system_frequency = 50                  -- System frequency (Hz)
R_dc_20 = 0.0176e-3                    -- Ω/m 
...
tan_delta = 1e-3                       -- Loss factor of XLPE (filled and unfilled) based on IEC 60287-1-1 Table 3
...
core_lay_length = 2750                 -- Lay length of core stranding (mm)
...
-- r_armour = armour_outer_diameter / 2 -- Not used. For the armour to be treated as cylinder the cross of the actual wires should be adjusted. CIGRE TB 908 Section 3.2.1
-- r_cable_outer = cable_outer_diameter / 2 -- Not used. Adjusted accordinly to the armour diameter based on its thickness.
...
min_angle = 33.8 -- Related to meshing
...
-- CIGRE TB 908 Section 3.2.1 
function armour_layer_thickness()
    local d_A = armour_outer_diameter - diameter_armour_wires
    local d_f = diameter_armour_wires
    local layup_armour = sqrt(1+(pi * d_A / armour_lay_length)^2)
    local t = layup_armour *number_armour_wires * d_f * d_f / (4*d_A)
    return t
end
...
function calculate_conductor_losses(conductor_temp)
  ...
  -- Factor related to non-magnetic and magnetic armour wires - CIGRE TB 880 Guidance Point 25 for magnetic armour wires and IEC 60287-1-1 Section 5.1.1
  ...
  R_ac = layup_factor * R_ac -- Attention here CIGRE 880 TB GP23
...
function calculate_armour_losses(armour_temp, R_con_ac, R_sheath, lambda_1_circ)
  ...
  elseif material_armour == "Galvanized Steel" then
    ...
    -- Calculation of lamba_2 - IEC 60287-1-1 Section 5.4.3.3.1
...
armour_thickness = armour_layer_thickness()
r_armour = r_cable_core_binder + armour_thickness
r_cable_outer = r_armour + (cable_outer_diameter-armour_outer_diameter)/2
...
domain_factor = 18 -- times the depth CIGRE TB 963 Guidance points 14,44 and 45 
domain_width_margin = burial_depth * domain_factor  -- margin on each side (mm)
domain_depth_below = domain_width_margin   -- below lowest cable point (mm)
